...
static Args parse_args(const int argc, char** argv){
  ...
  if(a.M_min<1 || a.M_max<1 || a.N<1 || a.K<1){ fprintf(stderr,"Bad numeric arg.\n"); exit(1); }
  return a;
}
...
static auto elapsed_ms(cudaEvent_t a, cudaEvent_t b){
  float ms=0.0f; CHECK_CUDA(cudaEventElapsedTime(&ms, a, b)); return ms;
}
...
auto run_dist_gemm(const Args& a){
  // CSV header
